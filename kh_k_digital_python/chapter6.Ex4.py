"""
[문제] 세준이는 피시방에서 아르바이트를 한다. 세준이의 피시방에는 1번부터 100번까지 컴퓨터가 있다.
...
[입력]첫째 줄에 손님의 수 N이 주어진다. N은 100보다 작거나 같다.
...
pc = [0] * 100                  # 0값으로 초기화 된 리스트 100개 생성.
cnt = 0
for e in seat_num:              # e는 고객이 앉고 싶어하는 좌석 번호.
    if pc[e - 1] != 0:          # pc[]의 index는 0부터 99까지. 0이 아니면 이미 예약된 자리임.
        cnt += 1
    else: pc[e - 1] = 1
print(cnt)
